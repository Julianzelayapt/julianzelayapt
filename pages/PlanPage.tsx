  ...
  if (!plan) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center bg-[var(--background)] text-[var(--foreground)]">
        <h1 className="text-4xl font-black uppercase mb-4 text-[var(--ios-text)]">{t.plan_not_found}</h1>
    ...
    <main className="min-h-screen bg-[var(--background)] px-6" style={{ paddingTop: '130px', paddingBottom: '64px' }}>
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <span className="font-bebas text-xl md:text-2xl tracking-widest uppercase translate-y-0.5">{t.back}</span>
        </Link>
            ...
            >
              ...
              <div className="prose prose-lg dark:prose-invert text-[var(--ios-text)] opacity-90 mb-12">
                ...
                <div className="mt-12 mb-12">
                   <h3 className="font-bebas text-3xl md:text-5xl text-[var(--ios-text)] tracking-wide uppercase mb-8">{t.what_you_get}</h3>
                   <div className="grid grid-cols-1 gap-4">
                      {plan.items.map((itemKey, i) => (
                        <motion.div 
                          ...
                          className="flex items-center gap-4 bg-[rgba(255,255,255,0.03)] backdrop-blur-sm p-5 border border-white/5 rounded-xl group hover:border-[#FFE50C]/50 hover:bg-[rgba(255,229,12,0.05)] transition-all duration-300"
                        >
                          <div className="flex-shrink-0 w-6 h-6 rounded-full bg-[#FFE50C]/10 flex items-center justify-center group-hover:bg-[#FFE50C] transition-colors duration-300">
                             <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" className="text-[#FFE50C] group-hover:text-black transition-colors duration-300">
                                <polyline points="20 6 9 17 4 12" />
                             </svg>
                          </div>
                          <span className="font-sans text-sm md:text-base uppercase tracking-widest font-bold opacity-80 text-[var(--ios-text)] group-hover:opacity-100 transition-opacity">
                            {t[itemKey]}
                          </span>
                        </motion.div>
                      ))}
              ...
              <div className="flex items-center justify-center gap-6 mt-4 opacity-50 text-[var(--ios-text)]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs uppercase font-bold tracking-wider">{t.secure_payment}</span>
                </div>
                <div className="flex items-center gap-2">
                   <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                   </svg>
                   <span className="text-xs uppercase font-bold tracking-wider">{t.immediate_access}</span>
                </div>
              </div>
            </motion.div>
          </div>
